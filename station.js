...
function buildDebrief() {
  ...
  gr.innerHTML = "";
  GRILLE.forEach(sec => {
    const d = sec.items.filter(i => checkedItems[i.id]).length;
    ...
    block.innerHTML = `<div class="db-section-head"><h4>${esc(sec.title)}</h4><span class="db-frac ${d === sec.items.length ? "full" : ""}">${d}/${sec.items.length}</span></div>${items}`;
