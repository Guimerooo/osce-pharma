...
/* Un cas est "modifié" seulement si son override diffère réellement de la base. */
function _normCaseContent(c) {
  return JSON.stringify({
    plainte: c.plainte, variant: c.variant, opening: c.opening,
    answers: (c.answers || []).map(a => ({ q: a.q, a: a.a })),
    sections: (c.sections || []).map(s => ({
      title: s.title,
      items: (s.items || []).map(it => (it && typeof it === "object" && it.subs && it.subs.length) ? { label: it.label, subs: it.subs } : (it && typeof it === "object" ? it.label : it))
    }))
  });
}
function caseIsModified(id) {
  const ov = loadAdminStore().overrides[id];
  if (!ov) return false;
  const baseCase = CASES.find(c => c.id === id);
  if (!baseCase) return false;
  return _normCaseContent(ov) !== _normCaseContent(baseCase);
}
...
function buildAdminList() {
  ...
  casesByPlainte().forEach(group => {
    ...
    group.cases.forEach(c => {
      ...
      const edited = base && caseIsModified(c.id);
      const badge = !base
